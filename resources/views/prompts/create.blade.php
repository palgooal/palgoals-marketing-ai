...
@section('content')
    <div class="mx-auto max-w-4xl">
        ...
        <div class="mb-6 rounded-xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-700 shadow-sm">
            ...
            <p class="mt-2">Use the system prompt for stable behavioral instructions and the user prompt template for
                runtime placeholders.</p>
            <p class="mt-2">Supported placeholders include <code>@{{ title }}</code>,
                <code>@{{ type }}</code>, <code>@{{ language }}</code>, and
                <code>@{{ tone }}</code>. Some workflows may also pass additional payload or context fields such
                as <code>@{{ context }}</code>, <code>@{{ goals }}</code>, or page-specific fields.</p>
        </div>
                ...
                <form method="POST" action="{{ route('prompts.store') }}" class="space-y-6">
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                        <div>
                            ...
                            <select id="module" name="module"
                                ...
                                @foreach (['content' => 'Content', 'offers' => 'Offers', 'plans' => 'Plans', 'analysis' => 'Analysis'] as $value => $label)
                                    <option value="{{ $value }}" @selected(old('module', 'content') === $value)>{{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="version" name="version" type="number" min="1"
                                class="mt-1 block w-full" :value="old('version', 1)" required />
                    ...
                    <div>
                        ...
                        <textarea id="system_prompt" name="system_prompt" rows="6"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('system_prompt') }}</textarea>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs text-gray-500">Use this template for runtime fields. Available placeholders
                            ...
                            or context values may also be available depending on the workflow.</p>
                        <textarea id="user_prompt_template" name="user_prompt_template" rows="10"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('user_prompt_template') }}</textarea>
